    ...
    with open(health_file, 'r') as f:
        ...
        ingest_client = QueuedIngestClient(
            KustoConnectionStringBuilder.with_azure_token_credential(
                ingest_url, creds))
        print(
            f"Ingesting health results from \
            {os.path.basename(health_file)} into \
            {ingest_url} at {database}/{health_table_name}")
        ingest_client.ingest_from_dataframe(
            df, IngestionProperties(
                database, health_table_name))


def ingest_debug_log(
        debug_file,
        creds,
        ingest_url,
        database,
    ...
    with open(debug_file, 'r') as f:
        ...
        print(
            f"Ingesting health results from \
            {os.path.basename(debug_file)} into \
            {ingest_url} at {database}/{debug_table_name}")
        ingest_client.ingest_from_dataframe(
            df, IngestionProperties(
...
def get_nhc_json_formatted_result(results_file):
    ...
    # check if GPU or CPU
    processor_cmd = "lspci | grep -iq NVIDIA"  # if not empty, then GPU
    ...
    if processor == "GPU":
        ...
        | grep -o 'ib_write_lb_mlx5_ib[0-7]: .*'"
        ib_write_lb_mlx5_ib_str = run_command(ib_write_lb_mlx5_ib_cmd)
        ib_write_lb_mlx5_ib_str = sorted(
            ib_write_lb_mlx5_ib_str.strip().split("\n"),
            key=natural_sort_key)
        ib_write_lb_mlx5_ib_str = '\n'.join(
            ib_write_lb_mlx5_ib_str)  # convert to string
        ...
        nccl_all_red_lb_cmd = f"cat {results_file} \
        | grep -o 'nccl_all_red_lb: .*'"
        nccl_all_red_lb_str = run_command(nccl_all_red_lb_cmd)

        data_string = "\n".join([ib_write_lb_mlx5_ib_str,
                                 H2D_GPU_str,
                                 D2H_GPU_str,
                                 P2P_GPU_str,
                                 nccl_all_red_str,
                                 nccl_all_red_lb_str])
        data_string = os.linesep.join(
            [s for s in data_string.splitlines() if s])  # remove empty lines
        result = {
            "IB_WRITE_GDR": {},
            "GPU_BW_HTD": {},
            "GPU_BW_DTH": {},
...
def ingest_results(
    ...
    job_name = results_file.replace(
        "\\", "/").split(".")[0].split("/")[-1]  # account for \ or / in path
    uuid = job_name if nhc_run_uuid == "None" else f"{nhc_run_uuid}-{job_name}"
    if uuid == "health":
        uuid = ""
    else:
        uuid = "-" + uuid
        # add the dash here instead of below
        # this way if 'uuid' is empty, we don't have a trailing dash
    full_uuid = f"nhc-{ts}{uuid}"

    vmSize_bash_cmd = "echo $( curl -H Metadata:true --max-time 10 -s \
    ...
    vmId_bash_cmd = "curl  -H Metadata:true --max-time 10 -s \
    ...
    physhost = run_command(
        "echo $(hostname) \"$(/opt/azurehpc/tools/kvp_client | grep Fully)\" \
        | cut -d ':' -f 3 | cut -d ' ' -f 2 | sed 's/\"//g'")
    if not physhost:
        physhost = "not Mapped"
    ...
    with open(results_file, 'r') as f:
        ...
        if "ERROR" in full_results:
            ...
            record['pass'] = False
            record['errors'] = "No Node Health Check \
            completed successfully or ERROR"
        ...
        ingest_client = QueuedIngestClient(
            KustoConnectionStringBuilder.with_azure_token_credential(
                ingest_url, creds))
        print(
            f"Ingesting results from \
            {os.path.basename(results_file)} into \
            {ingest_url} at {database}/{results_table_name}")
        ingest_client.ingest_from_dataframe(
            df, IngestionProperties(
                database, results_table_name))
...
def parse_args():
    ...
    parser.add_argument(
        "health_files",
        nargs="+",
        help="List of .health.log or .debug.log files to ingest")
    ...
    parser.add_argument(
        "--health_table_name",
        default="NodeHealthCheck",
        help="Kusto table name for health results")
    parser.add_argument(
        "--debug_table_name",
        default="NodeHealthCheck_Debug",
        help="Kusto table name for debug results")
    parser.add_argument(
        "--results_table_name",
        default="AzNhcRunEvents",
        help="Kusto table name for results")
    parser.add_argument(
        "--uuid",
        default="None",
        help="UUID to help identify results in Kusto table")
    parser.add_argument(
        "--identity",
        nargs="?",
        const=True,
        default=False,
        help="Managed Identity to use for authentication, \
        if a client ID is provided it will be used, \
        otherwise the system-assigned identity will be used. \
        If --identity is not provided DefaultAzureCredentials will be used.")
    ...
    try:
        ...
        if health_file.endswith(".health.log"):
            ...
            ingest_results(
                ...
                args.ingest_url,
                args.database,
                args.results_table_name,
                args.uuid)
        else:
            raise Exception(
                "Unsupported file, must be .health.log or \
                .debug.log produced by ./distributed_nhc.sb.sh, or \
                .log produced by run-health-checks.sh")
