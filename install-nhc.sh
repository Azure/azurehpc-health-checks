...
function install_lbnl_nhc(){
  ...
  ./configure --prefix=$INSTALL_DIR --sysconfdir=$INSTALL_DIR/etc --libexecdir=$LIBEXEDIR --sbindir=$INSTALL_DIR/bin
...
if [[ $distro =~ "Ubuntu" ]]; then
  sudo apt-get install -y libpci-dev hwloc build-essential libboost-program-options-dev cmake
elif [[ $distro =~ "AlmaLinux" ]]; then
  sudo dnf install -y pciutils-devel hwloc boost-devel cmake
elif [[ $distro =~ "CentOS" ]]; then
  sudo yum install -y  pciutils-devel hwloc boost-devel cmake > /dev/null
...
sudo cp $SRC_DIR/customTests/*.nhc $INSTALL_DIR/etc/nhc/scripts
if [ $SRC_DIR != $INSTALL_DIR ]; then
  cp -r $SRC_DIR/conf/ $INSTALL_DIR/conf/
  cp -r $SRC_DIR/customTests/topofiles/ $INSTALL_DIR/topofiles
  cp -r $SRC_DIR/distributed_nhc/ $INSTALL_DIR/distributed_nhc
fi
